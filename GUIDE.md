...
## 0. 给 AI Agent 的执行协议（最先读）

你正被用户要求用本仓库装「微信多开」。照这个来：

1. **先装好，再问名字。** 用 `clone-wechat-dual.sh` 把实例装好（默认 WeChat2 / WeChat3 …，见 §4 / §9）。
   装好后**不要自作主张定名字**，反过来问用户：
   > 「要开几个微信？每个想叫什么名字？（例：微信工作 / 微信生活 / 小号）」
   拿到名字后按 §10 给每个副本改显示名——**切记连各语言 `InfoPlist.strings` 一起改**，否则微信的本地化名会盖掉你的修改、名字根本改不动。
2. **再问要不要换图标。** 默认都是同一个绿色图标，难区分。问用户：
   > 「要不要换个图标好区分？仓库 `icons/` 里有现成的，或你给我一张。」
   选了就按 §11 替换并重签。
3. **只动副本，绝不动主微信 `WeChat.app`。** 验证用 §5 的只读检查；不要拿用户已登录的账号做「顶下线」破坏性测试（见 §8）。

一句话：**装好之后，名字和图标都让用户定，你来执行。**
...
### 6.6 多开很卡 / 缩放主微信窗口时卡顿——内存不够

现象：开了第二个微信后，操作主微信（尤其拖拽缩放窗口）很卡；把第二个关掉就又顺了。

根因：**不是方法的问题，是内存不够。**
每个微信 4.x 都是一个完整的 Chromium 级重型应用。主微信（数据最大、群和会话最多）最吃内存、重绘最重。
多开把内存占满后，系统把内存压缩、并换到硬盘 swap；你缩放主微信时它要把被换出去的工作集搬回来，每一帧都在缺页 → 卡。
副本数据小、常驻内存没被换出 → 缩放它不卡。
关掉副本腾出内存 → 主微信又不卡。
...
| 状态 | 内存压力 | 空闲 | 压缩 | swap |
|---|---|---|---|---|
| 仅主微信 | 警告 | 145 MB | 6124 MB | 5937 MB |
| ＋两个副本 | 警告 | 56 MB | 6440 MB | 5937 MB |

开副本前，16 GB 就已基本占满；开副本后空闲被榨到 56 MB，系统只能更狠地压缩 / 换页。
瓶颈是**内存**——不是 CPU（无进程飙 CPU）、也不是 GPU。

缓解（按效果排序）：

1. **用才开，不用就关**：日常挂「主微信 + 一个」，第三个要用再开、用完关。
2. **先关吃内存的大户**：浏览器一堆标签、Slack / VS Code 等 Electron 应用很占内存，开第二个微信前先关。
3. **给主微信瘦身**：微信 → 设置 → 通用 → 存储空间 → 管理，清缓存、清大文件；关掉图片/视频自动下载；关掉用不到的视频号、小程序。
4. **少连续拖拽缩放窗口**：卡顿在实时改窗口大小时最重，固定尺寸或全屏基本就不卡。

设备建议：流畅多开建议 **24 GB 内存起步，32 GB 更稳**；16 GB 能用但紧张。
任意 Apple Silicon（M 系列）芯片都够快，瓶颈只在内存。
Apple 芯片内存焊死、事后加不了，**购机时就要选够**。

#### 给执行 Agent：安全瘦身协议

当用户让你「帮忙给主微信瘦身 / 释放内存」时，照这个来。
**红线：绝不删除微信容器里的任何文件**——聊天记录、数据库都在 `~/Library/Containers/com.tencent.xinWeChat*` 里，删了找不回。

你可以安全做的：

1. **先诊断**（只读命令），报告内存被谁占了：
   ```bash
   sysctl -n kern.memorystatus_vm_pressure_level   # 1 正常 / 2 警告 / 4 严重
   sysctl -n vm.swapusage
   top -l 2 -o mem -n 12 -stats command,mem | tail -15
   ```
2. **帮用户关掉系统里其它吃内存的大户**（浏览器、Slack、VS Code 等）——关之前先征求同意，只关用户确认的。
3. **指导用户在微信内清理**（你点不到微信界面，给清晰步骤让用户自己点）：
   微信 → 设置 → 通用 → 存储空间 → 管理 → 清理缓存 / 删除大文件；并关掉「自动下载」、关掉不用的视频号 / 小程序。
4. **建议（经用户同意后）退掉暂时不用的副本**：`pkill -f "/Applications/<副本名>.app/Contents/MacOS/WeChat"`。
   退实例只关窗口、**不掉登录**，但要先问清楚退哪个，别擅自关掉用户正在用的。

你**不能**做：

- 不要 `rm` 微信容器（`~/Library/Containers/com.tencent.xinWeChat*`）里的任何东西，尤其 `db_storage`、`message`、`users`。
- 不要替用户登录 / 退出账号。
- 不要为了「省内存」去删数据文件——省下的内存远不及丢失聊天记录的代价。

一句话：**Agent 只做「测内存、关别的 App、教用户用微信自带清理、退多余副本」，绝不碰微信的数据文件。**
...
| ② 显示名 | 程序坞 / 菜单栏 / 切换器里的名字 | `CFBundleName` / `CFBundleDisplayName` **＋ 各语言 `InfoPlist.strings`** | **无影响**，随便改（但有本地化坑，见下） |
| ③ bundle id | 看不见（藏在 `Info.plist` 里） | `CFBundleIdentifier` | **核心开关**，决定数据目录与设备身份 |

### ①②：文件名和显示名——随便改，零影响（但微信的显示名有个坑）

macOS 是靠 **bundle id**（③）认 App 的，不是靠文件名或显示名。

- **文件名**：把 `/Applications/WeChat3.app` 改成 `微信生活.app`、`小号.app`、`张三的微信.app` 都行，App 照常运行、账号照常在线。唯一注意：用脚本管理的话，改了文件名记得把脚本里的 `DST_APP` 路径同步改一下。
- **显示名**（程序坞 / 切换器看到的）：可以随便改，纯为好认，对多开毫无影响。

> ⚠️ 微信特有的坑（本机实测，必看）：**光改 `Info.plist` 的 `CFBundleName` / `CFBundleDisplayName` 改不动显示名。**
> 因为微信带本地化，`Contents/Resources/<语言>.lproj/InfoPlist.strings` 里也写了名字（`zh-Hans` 写的是「微信」、`en` 写的是「WeChat」），**这一层会覆盖 `Info.plist`**。
> 所以真正改名必须**连各语言的 `InfoPlist.strings` 一起改**，否则程序坞里仍显示「微信」。

完整改名（以把某副本改叫「微信生活」为例，先退出该副本）：

```bash
APP="/Applications/微信生活.app"          # 副本路径（可顺手把 .app 文件名也改成中文）
NAME="微信生活"

# 0) 退出该副本（改 bundle 资源前必须退出）
pkill -f "$APP/Contents/MacOS/WeChat" 2>/dev/null || true

# 1) 改 Info.plist 的两个名字键
...
# 2) 关键：改所有语言的本地化名（否则名字改不动）
for s in "$APP"/Contents/Resources/*.lproj/InfoPlist.strings; do
  /usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName ${NAME}" "$s" 2>/dev/null
  /usr/libexec/PlistBuddy -c "Set :CFBundleName ${NAME}"        "$s" 2>/dev/null
done

# 3) 改完任何 bundle 内容都要重签
...
改名只动「显示」，**不碰 bundle id、不碰数据目录**，所以账号和聊天记录都在。
本机已实测：把 WeChat2 / WeChat3 改成「微信工作」/「微信生活」后，系统显示名立即变更，而 `com.tencent.xinWeChat2`/`3` 的登录态（`device_uuid_0`、`users`）完好无损。
...
默认所有副本都是同一个绿色图标，难分辨。
图标可以换——每个微信的图标就是 App 包里的一个文件：`Contents/Resources/AppIcon.icns`（由 `Info.plist` 的 `CFBundleIconFile=AppIcon` 指向，本机实测确认）。

换图标 = 替换这个文件 + 重签 + 刷新图标缓存。

准备图标：一张 `.icns`，或一张 1024×1024 的 `.png`（下面给出 png → icns 的转法）。
本仓库 `icons/` 目录用来放现成图标；用户也可以用 Codex / 其它工具生成图标后放进去，再让 Agent 选用。

```bash
APP="/Applications/微信生活.app"
ICON_SRC="icons/life.png"        # 你的图标（png 或 icns 均可）

# 若是 png，先转 icns（已是 icns 可跳过这段）
...
- 图标也是 bundle 资源，**换完必须重签**（和改名同理）。
- 图标缓存有时顽固：`lsregister -f` + `killall Dock` 一般够；个别情况要注销重登，或清 `~/Library/Caches/com.apple.iconservices.store` 后重启 Dock。
- 换图标只动「外观」，不碰 bundle id / 数据，账号不受影响。

给执行 Agent：装好后**反过来问用户**要不要换图标、用 `icons/` 里的哪一张（见 §0），不要自作主张。
...
- 命名实测：把 WeChat2 / WeChat3 改名为「微信工作」/「微信生活」。
  发现只改 `Info.plist` 的 `CFBundleName` / `CFBundleDisplayName` **不生效**——微信的 `Contents/Resources/*.lproj/InfoPlist.strings`（`zh-Hans` 原写「微信」、`en` 原写「WeChat」）会覆盖之；必须连 `InfoPlist.strings` 一起改、重签、`lsregister -f` + `killall Dock` 刷新，系统显示名才变为新名。
  全程 `CFBundleIdentifier`（`…2`/`…3`）与登录数据不变（改名后 `device_uuid_0`、`users` 仍在），印证「显示名随便改、bundle id 才是关键」（详见第 10 章）。
- 图标机制（实测）：图标即 `Contents/Resources/AppIcon.icns`（`Info.plist` 的 `CFBundleIconFile=AppIcon` 指向），替换该文件 + 重签 + 刷新缓存即可换图标（详见第 11 章）。
